window.onload = async function () {
  ...
  var tags = ["HP", "ASUS", "ACER", "LENOVO", "DELL"];
  for (var t of tags) addTags(t, "index3.php?search=" + t);
  ...
  for (var c of company)
    addCompany("img/company_pc/" + c, c.slice(0, c.length - 4));
  ...
  if (filters.length) {
    ...
    sanPhamPhanTich = phanTich_URL1(filters, true);
...
function addKhungSanPham(tenKhung, color, filter, len, ele) {
  ...
  var spResult = phanTich_URL1(filter, false);
  ...
  s +=
    `	</div>
                <a class="xemTatCa" href="index3.php?` +
    filter.join("&") +
    `" style="` +
    borderA +
    `">
...
function createLinkFilter(type, nameFilter, valueAdd) {
  ...
  var link = "index3.php"; //window.location.href.split('?')[0].replace('#', '');
...
function phanTich_URL1(filters, saveFilter) {
  var result = copyObject(list_products2);
  var result1 = copyObject(list_products1);
  var result2 = copyObject(list_products);
