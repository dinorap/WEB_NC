window.onload = async function () {
  ...
  autocomplete(
    ...
    list_products.concat(list_products1)
  );
  // thêm tags (từ khóa) vào khung tìm kiếm
  var tags = ["HP", "ASUS", "ACER", "LENOVO", "DELL"];
  for (var t of tags) addTags(t, "index2.php?search=" + t);
...
function addKhungSanPham(tenKhung, color, filter, len, ele) {
  ...
  s +=
    `	</div>
              <a class="xemTatCa" href="index2.php?` +
    filter.join("&") +
    `" style="` +
    borderA +
    `">
...
function createLinkFilter(type, nameFilter, valueAdd) {
  ...
  var link = "index2.php"; //window.location.href.split('?')[0].replace('#', '');
...
function phanTich_URL1(filters, saveFilter) {
  var result = copyObject(list_products1);
