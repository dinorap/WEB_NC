function User(username, pass, ho, ten, email, locker, products, donhang) {
  this.ho = ho || "";
  this.ten = ten || "";
  this.email = email || "";
  this.locker = locker || "F";
...
//toWeb() chuyển đối tượng thành một định dạng dùng cho web.
function Promo(name, value) {
  ...
  this.toWeb = function () {
    ...
    switch (this.name) {
      case "giamgia":
        contentLabel =
          `<i class="fa fa-bolt"></i> Giảm ` + this.value + `&#8363;`;
        break;
...
function addToWeb(p, ele, returnString) {
  ...
  var newLi =
    ...
    `
			<div class="tooltip">
				<button class="themvaogio" onclick="themVaoGioHang('` +
    p.masp +
    `', '` +
    p.name +
    `', '` +
